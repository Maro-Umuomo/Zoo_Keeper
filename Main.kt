...
fun main() {
    ...
    while (true){
        ...
        for (element: Char in userInput)
        {
            if (element.isDigit()) {
                index = element.digitToInt()
                validInput = true
                break
            }
        }

        if (validInput && index in animals.indices) {
            println(animals[index])
        } else {
            println("Invalid selection. Number must be between range 0 - 5. Or type exit to stop program")
        }
    }
